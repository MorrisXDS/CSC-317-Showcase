...
#### You may check each ppm file and the ppm file generated by external stl is wind-dragon.rgb.ppm
## Description:
    1. A customized json file to display the output of assignment 3 on the wind-dragon.stl
    2. The current code runs siginificantly faster than the base assignment 3 we started with
        Time taken to produce a complex ppm from a complext stl has been reduced from several
        hours to days has been reduced to several minutes to hours.
    3. It combines the blinn-phong model and relections from assignment 3. Meanwhile it uses 
        AABB tree to find ray hits. This improves ray hit lookup from O(n) to O(log (n))
